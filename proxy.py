...
def load_preset():
    ...
    preset_dir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "presets", preset_name
    )
            ...
            if hasattr(preset_module, var):
                ...
                if not hasattr(config, var) or getattr(config, var) != preset_value:
                    ...
                    if old_value is None:
                        ...
                        old_truncated = old_val[:100] + (
                            "..." if len(old_val) > 100 else ""
                        )
                        new_truncated = new_val[:100] + (
                            "..." if len(new_val) > 100 else ""
                        )
                        debug_print(
                            f"Preset '{preset_name}' changed {var} from {old_truncated} to {new_truncated}"
                        )
        if changes_made:
...
def handle_image_request(url):
    ...
    if cached_url:
        return send_from_directory(
            CACHE_DIR, os.path.basename(cached_url), mimetype="image/gif"
        )
    else:
...
def handle_override_extension(scheme):
    ...
    if override_extension:
        ...
        if extension_name in extensions:
            ...
            print(
                f"Warning: Override extension '{extension_name}' not found. Resetting override."
            )
            override_extension = None
...
def check_override_status(extension_name):
    global override_extension
    if (
        hasattr(extensions[extension_name], "get_override_status")
        and not extensions[extension_name].get_override_status()
    ):
        override_extension = None
...
def handle_matching_extension(matching_extension):
    global override_extension
    debug_print(
        f"Handling request with matching extension: {matching_extension.__name__}"
    )
    response = matching_extension.handle_request(request)

    if (
        hasattr(matching_extension, "get_override_status")
        and matching_extension.get_override_status()
    ):
...
def process_response(response, url):
    ...
    if should_transcode:
        ...
        debug_print(
            f"Content type {content_type} should not be transcoded, passing through unchanged"
        )
    ...
    try:
        ...
        if hasattr(e, "args") and e.args:
            ...
            if (
                ...
                return abort(
                    502,
                    f"{ERROR_HEADER} DNS lookup failed for {url}. Please check the domain name.",
                )
